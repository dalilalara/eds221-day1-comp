...
### Set's global settings to echo code, not print messages, and not print warning messages
...
### Calls for data set from working directory to data to subfile data-raw 
## Here function starts at root level of working directory 
```{r}
...
## Basic exploration
Provides basic quick overview of data 
```{r}
# An overview:
...
### clean_names function converts into coder friendly name make the order you write code in be more intuitive the order you think things are happening 

### Pipes examples 
 object %>% filter() %>% select()  %>% mutate()
```{r}
...
names(toolik_weather)
```
### command shift m shortcut for pipe %>% 
...
## Export a figure
### ggsave(here::here("folder", "nameofnewsavedfile.png"), height = 6, width = 7 )
```{r}
